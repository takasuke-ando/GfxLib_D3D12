                                     ...
                                     _In_ D3D12_CPU_DESCRIPTOR_HANDLE textureView )
{
...

    {
		...
		if (SUCCEEDED(hr))
		{
			...
			/*
			const UINT64 uploadBufferSize = d3dx12::GetRequiredIntermediateSize(*texture, 0, subresourceCount);


			CommandList *initCmdList = CoreSystem::GetInstance()->GetResourceInitCommandList();

			d3dx12::UpdateSubresources(initCmdList->GetD3DCommandList(), *texture, d3dResForUpload, 0, 0, subresourceCount, initData.get());
			initCmdList->ResourceTransitionBarrier(*texture, ResourceStates::CopyDest, ResourceStates::GenericRead);
